  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        process.env.REACT_APP_API_BASE_URL + "/merchandise",
        formDataToSend,
        {
      ...
      if (error.response) {
        if (error.response.data.includes("<!DOCTYPE html>")) {
          swal("Error", "The server responded with an error page. Please check the server logs.", "error");
        } else {
          swal("Error", `Failed to add product: ${error.response.data.message || 'Unknown error'}`, "error");
        }
      } else if (error.request) {
        swal("Error", "The server did not respond. Please try again later.", "error");
      } else {
  ...
  return (
    <div className="container-fluid mt-4 col-11 mx-auto">
      <div className="card p-4 shadow border">
        <h2 className="fw-bold text-center text-warning mb-4">Add Product</h2>
      ...
      .map((cat) => (
        <option key={cat.id} value={cat.id}>
          ...
          <div className="mb-3">
            ...
            <button type="button" className="btn btn-warning mt-3" onClick={addVariant}>
              Add Variant
            </button>
          </div>
